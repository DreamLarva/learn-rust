...
// 除了引用数据没有任何其他特殊功能。数据原来哪里,现在还是在哪里(堆,栈),且数据
...
pub fn ch15_01_box() {
    ...
    // 其他更为复杂的递归类型.大部分在Rust很多场景中很有用, 不过通过以cons list 作为你开始,我我们可以探索如何使用 box 毫不费力的定义一个递归数据类型。
...
// 当可以确保在运行时遵守借用规则,即使编译器不能保证的情况下,可以选择使用那些运用内部可变性模式的了类型.
// 所涉及的unsafe 代码将被封禁安全的 API 中,而外部类型任然是不可变的

// 通过 RefCell<T> 在运行时检查借用规则
// 对于引用和Box<T> ,借用规则的不可变性作用域编译时.对于 RefCell<T>,这些不可变性作用域运行时.
// 对于引用,如果违反这些规则,就会得到一个编译错误. 而对于RefCell<T>,如果违反这些规则程序会 panic并退出.
...
// Box<T> 允许在编译时执行不可变或可变借用检查;Rc<T> 仅允许在编译是执行不可变借用检查;RefCell<T> 允许在运行时执行不可变或可变借用检查
// 因为 RefCell<T> 允许在运行时执行可变借用检查,所以我们可以在即便RefCell<T> 自身是不可变的情况下修改内部的值
...
mod tests {
    ...
    struct MockMessenger {
        // 如果不用 RefCell 的话, 那么修改很多地方都要改成 mut
        // 但是用了 RefCell 直接borrow 就能单独操作
